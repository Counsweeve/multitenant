...
class SecurityConfig(BaseSettings):
    ...
    MAGIC_LINK_ENCRYPTION_KEY: str = Field(
        description="AES encryption key for magic link (32 bytes, base64 encoded). "
                    "If not set, will derive from SECRET_KEY. "
...
class WorkspaceConfig(BaseSettings):
    ...
    DEFAULT_TENANT_PROJECT_SPACE: Optional[str] = Field(
        description="Default project space name for newly created tenants",
        default=None,
    )
...
class LoginConfig(BaseSettings):
    ...
    MAGIC_LOGIN_CORS_ALLOWED_ORIGINS: str = Field(
        description="comma separated origins allowed to access /api/magic-login",
        default="",
    )

    EXTERNAL_SSO_USER_INFO_URL: str = Field(
        description="External SSO user info API URL for external SSO login integration",
        default="",
    )
