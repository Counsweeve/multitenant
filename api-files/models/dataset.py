...
class ProvinceScene(Base):
    __tablename__ = "province_scene"
    __table_args__ = (
        sa.PrimaryKeyConstraint("id", name="province_scene_pkey"),
        sa.Index("idx_province_scene_unique", "province_tag", "scene_tag", unique=True),
    )

    id: Mapped[int] = mapped_column(sa.BigInteger, primary_key=True, autoincrement=True)
    province_tag: Mapped[str] = mapped_column(String(8), nullable=False)
    chi_pro: Mapped[str] = mapped_column(String(32), nullable=False)
    scene_tag: Mapped[str] = mapped_column(String(32), nullable=False)
    chi_scene: Mapped[str] = mapped_column(String(64), nullable=False)
    specialty_tag: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)
    chi_spe: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    chi_spec: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
