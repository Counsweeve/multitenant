...
class AppModelConfig(Base):
    ...
    marked_name = mapped_column(sa.Text)
    marked_comment = mapped_column(sa.Text)
    publish_square = mapped_column(sa.Boolean, nullable=False, server_default=sa.text("false"))
...
class OperationLog(Base):
    ...
    __table_args__ = (
        ...
        sa.Index("operation_log_tenant_idx", "tenant_id"),
        sa.Index("operation_log_account_idx", "account_id"),
        sa.Index("operation_log_action_idx", "action"),
        sa.Index("operation_log_created_at_idx", "created_at"),
    )

    id = mapped_column(StringUUID, server_default=sa.text("uuid_generate_v4()"))
    tenant_id = mapped_column(StringUUID, nullable=False, comment="租户ID")
    account_id = mapped_column(StringUUID, nullable=False, comment="操作人ID")
    account_email = mapped_column(String(255), nullable=False, comment="操作人邮箱")
    action: Mapped[str] = mapped_column(String(100), nullable=False, comment="操作类型")
    action_name: Mapped[str] = mapped_column(String(255), nullable=False, comment="操作类型名称")
    content = mapped_column(sa.JSON, nullable=True, comment="操作内容详情")
    description: Mapped[str] = mapped_column(sa.Text, nullable=True, comment="操作描述")
    ip_address: Mapped[str] = mapped_column(String(45), nullable=False, comment="操作IP地址")
    user_agent: Mapped[str] = mapped_column(sa.Text, nullable=True, comment="用户代理")
    created_at = mapped_column(sa.DateTime, nullable=False, server_default=func.current_timestamp(), comment="操作时间")
    updated_at = mapped_column(sa.DateTime, nullable=False, server_default=func.current_timestamp(), comment="更新时间")
...
class ApiToken(Base):
    ...
    last_used_at = mapped_column(sa.DateTime, nullable=True)
    created_at = mapped_column(sa.DateTime, nullable=False, server_default=func.current_timestamp())
    selected_app_id = mapped_column(StringUUID, nullable=True)  # 选择的特定应用ID
    expires_at = mapped_column(sa.DateTime, nullable=True)  # 有效期
    description = mapped_column(String(255), nullable=True)  # 密钥描述
...
class ApiKey(Base):
    """
    目前支持agents/workflows 之后可以兼容添加新类型
    """
    __tablename__ = "api_keys"
    __table_args__ = (
        sa.PrimaryKeyConstraint("id", name="api_key_pkey"),
        sa.Index("api_key_tenant_id_idx", "tenant_id"),
        sa.Index("api_key_resource_idx", "tenant_id", "resource_type", "resource_id"),
        sa.UniqueConstraint("key", name="api_key_key_unique")
    )
    ...
    validity_type: Mapped[str] = mapped_column(String(20), nullable=False)
    start_date: Mapped[Optional[datetime]] = mapped_column(sa.DateTime, nullable=True)
    end_date: Mapped[Optional[datetime]] = mapped_column(sa.DateTime, nullable=True)
    authorized_person: Mapped[str] = mapped_column(sa.String(255), nullable=False)
    description: Mapped[str] = mapped_column(sa.Text, server_default=sa.text("''::character varying"))
    ...
    @classmethod
    def generate_key(cls, prefix: str = "dify") -> str:
        """
        Generate a secure API key
        Format: prefix_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
        """
    ...
    def masked_key(self) -> str:
        """
        Return masked key for display (e.g., abc***cba)
        """
        if len(self.key) <= 6:
            return self.key
        return f"{self.key[:3]}***{self.key[-3:]}"
...
class ApiKeyUsageLog(Base):
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint("id", name="api_key_usage_log_pkey"),
        sa.Index("api_key_usage_log_key_idx", "api_key_id"),
        sa.Index("api_key_usage_log_time_idx", "created_at"),
    )

    id: Mapped[str] = mapped_column(StringUUID, server_default=sa.text("uuid_generate_v4()"))
    api_key_id: Mapped[str] = mapped_column(StringUUID, nullable=False)
    tenant_id: Mapped[str] = mapped_column(StringUUID, nullable=False)
    resource_type: Mapped[str] = mapped_column(sa.String(50), nullable=False)
    resource_id: Mapped[str] = mapped_column(StringUUID, nullable=False)
    request_method: Mapped[str] = mapped_column(sa.String(10), nullable=False)
    request_path: Mapped[str] = mapped_column(sa.String(500), nullable=False)
