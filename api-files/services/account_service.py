...
class AccountService:
    ...
    def create_account(
        ...
        """create account"""
        print("FeatureService.get_system_features().is_allow_register:",FeatureService.get_system_features().is_allow_register)
...
class TenantService:
    ...
    @staticmethod
    def create_owner_tenant_if_not_exist2(
        account: Account, name: Optional[str] = None, is_setup: Optional[bool] = False, role_id: Optional[str] = None,
        tenant_id: Optional[str] = None, data_role:Optional[str] = None
    ):
        available_ta = (
            db.session.query(TenantAccountJoin)
            .filter_by(
        ...
        TenantService.create_tenant_member2(tenant, account, data_role)
    ...
    @staticmethod
    def create_owner_tenant_if_not_exist_with_tenant_name(account: Account, tenant_name: Optional[str] = None, is_setup: Optional[bool] = True):

        existing_tenant = db.session.query(Tenant).first()
        admin_email = "admin@dify.com"

        if not existing_tenant:
            # No tenant exists in the system, create admin user and tenant first
            ...
            if not admin_account:
                ...
                admin_account = AccountService.create_account(
                    ...
                    is_setup=True,  # Use is_setup=True to bypass feature checks
                )
                logger.info(f"Created admin account: {admin_email}")
            else:
                logger.info(f"Admin account already exists: {admin_email}")

            # Create tenant with tenant_name for admin
            # During system initialization (no existing tenant), skip workspace license check
            # and create the initial admin tenant
            admin_tenant = TenantService.create_tenant(name=tenant_name, is_setup=True)
            TenantService.create_tenant_member(admin_tenant, admin_account, role="admin")
            admin_account.current_tenant = admin_tenant
            db.session.commit()
            tenant_was_created.send(admin_tenant)
            logger.info(f"Created initial tenant '{tenant_name}' with admin account")

            # If the current account is the admin account, we're done
        ...
        logger.info(f"available_ta:{available_ta}")
        if available_ta:
            return
        ...
        if not tenant:
            tenant = TenantService.create_tenant(name=tenant_name, is_setup=is_setup)
    ...
    @staticmethod
    def create_tenant_member2(tenant: Tenant, account: Account, data_role: str = None) -> TenantAccountJoin:
        """Create tenant member"""
        ta = db.session.query(TenantAccountJoin).filter_by(tenant_id=tenant.id, account_id=account.id).first()
        if not ta:
            ta = TenantAccountJoin(tenant_id=tenant.id, account_id=account.id, role = data_role)
            db.session.add(ta)
        db.session.commit()
        return ta
