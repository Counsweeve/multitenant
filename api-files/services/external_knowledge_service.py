...
from models.dataset import (
    ...
    ExternalKnowledgeBindings, ProvinceScene,
)
...
class ExternalDatasetService:
    ...
    def check_endpoint_and_api_key(settings: dict):
        ...
        logging.error("parsed_url: %s", parsed_url)
    ...
    @staticmethod
    def fetch_external_knowledge_retrieval_kl(
        tenant_id: str,
        dataset_id: str,
        query: str,
        external_retrieval_parameters: dict,
        ...
        province_scene = (
            db.session.query(ProvinceScene).
            filter_by(id = external_knowledge_binding.external_knowledge_id)
            .first()
        )
        tags = []
        if province_scene:
            tag_dict = {
                "scene_tag": province_scene.scene_tag,
                "province_tag": province_scene.province_tag,
                "specialty_tag": province_scene.specialty_tag,
                "chi_scene": province_scene.chi_scene,
                "chi_pro": province_scene.chi_pro,
                "chi_spe": province_scene.chi_spe
            }
            tags = [tag_dict]
        if not external_knowledge_api:
            raise ValueError("external api template not found")
        ...
        if response.status_code == 200:
            ...
            if not context:
                return []
            return ExternalDatasetService.convert_context_to_records(context)
        return []

    @staticmethod
    def convert_context_to_records(context: list[dict[str, Any]]) -> list[dict[str, Any]]:

        records: list[dict[str, Any]] = []
        for item in context:
            doc_name = item.get("doc_name", "") if item else ""
            url = item.get("url", "") if item else ""
            score = item.get("score") if item else None
            try:
                score_value: float = float(score) if score is not None else 1.0
            ...
            record = {
                ...
                "score": score_value,
                "title": doc_name,
                "content": item.get("text", "") if item else "",
            }
