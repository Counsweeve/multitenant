...
from fields.dataset_fields import dataset_detail_fields, dataset_query_detail_fields, province_scene_fields
from fields.document_fields import document_status_fields
from libs.login import login_required
from models import ApiToken, Dataset, Document, DocumentSegment, UploadFile
from models.dataset import DatasetPermissionEnum, ProvinceScene
...
class ProvinceSceneListApi(Resource):
    # @setup_required
    # @login_required
    # @account_initialization_required
    def get(self):
        parser = reqparse.RequestParser()
        parser.add_argument("province_tag", type=str, location="args")
        parser.add_argument("scene_tag", type=str, location="args")
        parser.add_argument("specialty_tag", type=str, location="args")
        args = parser.parse_args()

        query = db.session.query(ProvinceScene)
        if args.get("province_tag"):
            query = query.where(ProvinceScene.province_tag == args["province_tag"])
        if args.get("scene_tag"):
            query = query.where(ProvinceScene.scene_tag == args["scene_tag"])
        if args.get("specialty_tag"):
            query = query.where(ProvinceScene.specialty_tag == args["specialty_tag"])

        province_scenes = query.order_by(ProvinceScene.province_tag, ProvinceScene.scene_tag).all()
        return {
            "success": True,
            "data": marshal(province_scenes, province_scene_fields)
        }, 200
...
api.add_resource(ProvinceSceneListApi, "/datasets/province-scenes")
